	...
	if ( check_ajax_referer( 'reservation', 'rn' ) ) {
		...
		if ( $reservations->found_posts > 0 ) {
			...
			if ( ! $wp_error ) {
				...
				$order->add_product( wc_get_product( 79 ), 1 ); // product id, amount koita product add korbo
				$order->set_customer_note( $reservation_id ); // order ar sathe reservation ar link
